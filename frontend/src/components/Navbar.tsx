      ...
      <div className={cn(
        ...
        <div className="flex items-center justify-between h-12">
          ...
          <div className="hidden md:flex items-center bg-white/5 backdrop-blur-xl rounded-full p-1 border border-white/5">
            <NavLink href="/dashboard" active={pathname === '/dashboard'} icon={LayoutDashboard}>Dashboard</NavLink>
            <NavLink href="/explore" active={pathname === '/explore'} icon={Compass}>Explore</NavLink>
            <NavLink href="/identity" active={pathname === '/identity'} icon={Info}>Identity</NavLink>
            <NavLink href="/access" active={pathname === '/access'} icon={CreditCard}>Access</NavLink>
          </div>
          ...
          <div className="flex items-center space-x-1">
            <motion.button 
              ...
              whileTap={{ scale: 0.95 }}
              className="hidden md:block p-2 text-surface-400 hover:text-white transition-colors hover:bg-white/5 rounded-xl"
            >
            ...
            <motion.button 
              ...
              className="md:hidden p-2 text-surface-400 hover:text-white transition-colors hover:bg-white/5 rounded-xl ml-1"
            >
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            ...
            >
              <div className="pt-4 pb-2 border-t border-white/10 mt-2 space-y-1">
                <MobileNavLink href="/dashboard" active={pathname === '/dashboard'} icon={LayoutDashboard} onClick={() => setMobileMenuOpen(false)}>Dashboard</MobileNavLink>
                <MobileNavLink href="/explore" active={pathname === '/explore'} icon={Compass} onClick={() => setMobileMenuOpen(false)}>Explore</MobileNavLink>
                <MobileNavLink href="/identity" active={pathname === '/identity'} icon={Info} onClick={() => setMobileMenuOpen(false)}>Identity</MobileNavLink>
                <MobileNavLink href="/access" active={pathname === '/access'} icon={CreditCard} onClick={() => setMobileMenuOpen(false)}>Access</MobileNavLink>
                
                <div className="h-px bg-white/10 my-2" />
                
                <MobileNavLink href="/dashboard/settings?tab=profile" icon={User} onClick={() => setMobileMenuOpen(false)}>Profile</MobileNavLink>
                <MobileNavLink href="/dashboard/settings" icon={Settings} onClick={() => setMobileMenuOpen(false)}>System Settings</MobileNavLink>
                
                <button 
                  onClick={() => { logout(); setMobileMenuOpen(false); }}
                  className="w-full flex items-center space-x-3 px-4 py-3 rounded-xl text-red-400 hover:bg-red-500/10 transition-colors text-left"
                >
...
function MobileNavLink({ href, children, active = false, icon: Icon, onClick }: { href: string, children: React.ReactNode, active?: boolean, icon: any, onClick?: () => void }) {
  return (
    <Link 
      href={href}
      onClick={onClick}
      className={cn(
        ...
        active 
          ? "bg-beyond-purple/20 text-white border border-beyond-purple/30" 
          : "text-surface-300 hover:text-white hover:bg-white/5"
      )}
    >
      <Icon className={cn("w-4 h-4", active ? "text-beyond-purple" : "text-surface-400")} />
      <span>{children}</span>
    </Link>
  );
}
